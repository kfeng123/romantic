...
#周六节日dummy
myFeature$jiaqi6=(1:427 %in% c(279,306,335))+0
#周日节日dummy
myFeature$jiaqi7=(1:427 %in% c(280,307,336))+0
#几号
myFeature$jihao=as.numeric(format(finalTot$report_date,format="%d"))
#假期前1天特征
myFeature$qian1=(1:427 %in% c(80,184,278,304,334))+0
#假期前2天特征
myFeature$qian2=(1:427 %in% c(79,183,277,303,333))+0
#1:427特征
myFeature$t=1:427
###########测试集特征构建
...
match(jiaqitemp,finalTot$report_date)
myFeature$shang=(finalTot$total_purchase_amt/gam(purchase~s(t,k=14),data=myFeature)$fitted.values)

#############迭代
tempFit=gam(shang~S1+S2+S3+S4+S5+S6+yuechu+buxiu+jiaqiN+jiaqi6+jiaqi7+s(jihao,k=31),data=myFeature)
#2013年十一93-99
#过年215-221
#过年前5天都不太正常
tuX=c(93:99,210:221)
myFeature$shang[1:427%in%tuX]=tempFit$fitted.values[1:427%in%tuX]
myPlotTot(myFeature$shang)
lines(tempFit$fitted.values,col="red")
myPlotTot(myFeature$shang-tempFit$fitted.values)
myPlotTot(finalTot$total_purchase_amt)
############迭代结束
#预测
a=predict(gam(purchase~s(t,k=14),data=myFeature),toP)
b=predict(tempFit,toP)

(a*b)[30]+(myFeature$shang[92]-tempFit$fitted.values[92])*a[30]
(a*b)[29]+(myFeature$shang[91]-tempFit$fitted.values[91])*a[29]


plot(a*b,type="l")

lines(finalTot$total_purchase_amt[400:427],col="red")
plot(c(gam(purchase~s(t,k=14),data=myFeature)$fitted.values*tempFit$fitted.values,a*b),type="l")
lines(finalTot$total_purchase_amt,col="red")
lines(c(gam(purchase~s(t,k=14),data=myFeature)$fitted.values,a))
purchase=a*b
purchase[30]=(a*b)[30]+(myFeature$shang[92]-tempFit$fitted.values[92])*a[30]
purchase[29]=(a*b)[29]+(myFeature$shang[91]-tempFit$fitted.values[91])*a[29]
...
match(jiaqitemp,finalTot$report_date)
myFeature$shang=(finalTot$total_redeem_amt/gam(redeem~s(t,k=11),data=myFeature)$fitted.values)
plot(gam(redeem~s(t,k=11),data=myFeature))

#############迭代
tempFit=gam(shang~S1+S2+S3+S4+S5+S6+buxiu+jiaqiN+jiaqi6+jiaqi7+qian1+s(jihao,k=31),data=myFeature)
#2013年十一93-99
#过年215-221
#过年前5天都不太正常
tuX=c(93:99,210:221)
...
############迭代结束
#预测
a=predict(gam(redeem~s(t,k=11),data=myFeature),toP)
b=predict(tempFit,toP)
plot(a*b,type="l")
lines(finalTot$total_purchase_amt[400:427],col="red")
plot(c(gam(redeem~s(t,k=11),data=myFeature)$fitted.values*tempFit$fitted.values,a*b),type="l")
lines(finalTot$total_redeem_amt,col="red")
lines(c(gam(redeem~s(t,k=11),data=myFeature)$fitted.values,a))
redeem=a*b


write.csv(as.integer(purchase),"purchase3.csv",row.names=FALSE)
write.csv(as.integer(redeem),"redeem3.csv",row.names=FALSE)
